...
def create_keyboard_page_users(
    ...
    for i, user in enumerate(users):
        user_buttons.append(
            InlineKeyboardButton(
                text=str(i + 1),
                callback_data=ModerationCallback(action="view", tg_id=user.tg_id, page=page).pack(),
            )
        )
...
def create_keyboard_view_user(
    ...
    builder.button(
        text="Удалить",
        callback_data=ModerationCallback(action="confirm_delete", tg_id=tg_id, page=page).pack(),
    )

    if search:
        builder.button(text="Назад", callback_data=AdminCallback(action="back_to_search").pack())
    else:
        builder.button(text="Назад", callback_data=AdminCallback(action="page", page=page).pack())
...
def create_keyboard_confirm_delete(page: int, tg_id: int) -> InlineKeyboardMarkup:
    ...
    builder.button(
        text="Да",
        callback_data=ModerationCallback(action="delete", tg_id=tg_id, page=page).pack(),
    )
    builder.button(
        text="Нет",
...
def create_stats_keyboard(current: str) -> InlineKeyboardMarkup:
    ...
    builder.button(text="Назад", callback_data=AdminCallback(action="back_to_panel").pack())
