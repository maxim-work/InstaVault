...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser: ArgumentParser) -> None:
        parser.add_argument(
            "username", type=str, help="Username of the new owner"
        )
        parser.add_argument(
    ...
    def handle(self, *_args: Any, **options: Any) -> None:
        ...
        try:
            new_owner = CustomUser.objects.get(username=username)
        ...
        if new_owner == current_owner:
            self.stdout.write(
                self.style.ERROR(f'User "{username}" is already the owner')
            )
            return
        ...
        if not new_owner.is_superuser or not new_owner.is_staff:
            self.stdout.write(
                self.style.ERROR(
                    "The new owner must be a superadmin and staff"
                )
            )
            return
        ...
        if not force:
            ...
            confirm = input(
                f'Transfer ownership to "{username}"? (y/n): '
            )
        ...
        self.stdout.write(
            self.style.SUCCESS(
                f'Owner rights transferred to "{username}"'
            )
        )
        self.stdout.write(
    ...
    def _create_owner(self, username: str, force: bool) -> None:
        try:
        ...
        if not user.is_superuser or not user.is_staff:
            self.stdout.write(
                self.style.ERROR(
                    "User must be a superadmin and staff to become owner"
                )
            )
            return
