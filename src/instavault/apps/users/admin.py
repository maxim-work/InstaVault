...
class UserSettingsInline(admin.StackedInline):  # type: ignore[type-arg]
    model = UserSettings
    can_delete = False
    extra = 0
    max_num = 1
...
class CustomUserAdmin(UserAdmin):  # type: ignore[type-arg]
    ...
    transfer_owner.short_description = "Передать права владельца"  # type: ignore[attr-defined]
    ...
    send_telegram_message.short_description = "Отправить Telegram-сообщение"  # type: ignore[attr-defined]
    ...
    send_email_message.short_description = "Отправить Email-письмо"  # type: ignore[attr-defined]

    def _ban_unban_handler(
        self,
        request: HttpRequest,
        queryset: QuerySet[CustomUser],
        flag: str,
        filter_kwargs: dict[str, bool],
        error_message: str,
    ) -> HttpResponse | None:
    ...
    ban_users.short_description = "Забанить пользователя(ей)"  # type: ignore[attr-defined]
    ...
    unban_users.short_description = "Разбанить пользвателя(ей)"  # type: ignore[attr-defined]
    ...
    def get_queryset(self, request: HttpRequest) -> QuerySet[CustomUser]:
        ...
        if user.is_superuser and not user.is_owner:
    ...
    def get_email_link(self, obj: CustomUser) -> str:
        url = reverse("admin:users_customuser_change", args=[obj.pk])
        return format_html('<a href="{}">{}</a>', url, obj.email)

    get_email_link.short_description = "Email"  # type: ignore[attr-defined]
    get_email_link.admin_order_field = "email"  # type: ignore[attr-defined]

    def email_masked(self, obj: CustomUser) -> str:
        if not obj.email:
            return "-"
    ...
    email_masked.short_description = "Email"  # type: ignore[attr-defined]

    def telegram_status(self, obj: CustomUser) -> str:
        return "✅" if obj.telegram_id else "❌"

    telegram_status.short_description = "Telegram"  # type: ignore[attr-defined]
...
def confirm_ownership_transfer(request: HttpRequest) -> HttpResponse:
    user = get_user(request)
...
@staff_member_required
def send_telegram_message_view(request: HttpRequest) -> HttpResponse:
    user = get_user(request)
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            for u in users:
                try:
                    print(f"[TELEGRAM] Отправка сообщения пользователю {u.username}")
                    ...
                    log_action(
                        ...
                        category="admin",
                        details=f"Sent message to {u.username}: {message[:200]}",
                        request=request,
                    )

                except Exception as e:  # noqa: BLE001
                    failed_users.append(f"{u.username} (ошибка: {e!s})")
...
@staff_member_required
def send_email_message_view(request: HttpRequest) -> HttpResponse:
    user = get_user(request)
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            for u in users:
                try:
                    print(f"[EMAIL] Отправка сообщения пользователю {u.username}")
...
class BanOperationForm(Form):
    ...
    permanent = BooleanField(
        widget=CheckboxInput(attrs={"class": "ban-checkbox"}),
    ...
    def __init__(self, *args: Any, **kwargs: Any) -> None:
        self.flag: str = kwargs.pop("flag", "ban")
    ...
    def clean(self) -> dict[str, Any]:
        ...
        if self.flag == "ban" and not permanent and hours == 0 and days == 0:
            self.add_error(
                "hours",
                "Укажите срок бана или выберите перманентный бан",
            )
            self.add_error(
                "days",
                "Укажите срок бана или выберите перманентный бан",
...
@staff_member_required
def ban_operation_view(request: HttpRequest) -> HttpResponse:
    user = get_user(request)
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            for u in users:
                try:
                    if flag == "ban":
                        u.ban(reason, days, hours, permanent)
                        log_action(
                            user=u,
                            performed_by=user,
                            action="ban",
                            category="admin",
                            details=(
                                f"Ban via admin panel, reason: {reason}, "
                                f"{days}d {hours}h, permanent: {permanent}"
                            ),
                            request=request,
                        )
                    else:
            ...
            if success_count > 0:
                action_ru = "Забанено" if flag == "ban" else "Разбанено"
                messages.success(
                    request,
                    f"{action_ru} {success_count} из {count} пользователей",
                )

            if failed_users:
                action_ru2 = "забанить" if flag == "ban" else "разбанить"
                messages.warning(
                    request,
                    f"Не удалось {action_ru2}: {', '.join(failed_users)}",
                )
...
class AppealAdmin(admin.ModelAdmin):  # type: ignore[type-arg]
    list_display = (
        "username",
        "contact",
        "status",
    ...
    fieldsets = (
        (None, {"fields": ("username", "contact", "status", "created_at")}),
        ("Сообщение", {"fields": ("message",)}),
        ("Действия", {"fields": ("action_buttons",)}),
    )
    ...
    def action_buttons(self, obj: Appeal) -> str:
        if obj.status != "new":
            return "-"
        ...
        return format_html(
            '<a class="button" href="{}" '
            'style="background: #28a745; color: white;">Разбанить</a>&nbsp;'
            '<a class="button" href="{}" '
            'style="background: #dc3545; color: white;">Отклонить</a>',
            unban_url,
            reject_url,
        )

    action_buttons.short_description = "Действия"  # type: ignore[attr-defined]
    ...
    def approve_appeal(
        ...
        log_action(
            ...
            category="admin",
            details=f"Appeal #{appeal.pk} approved for {user.username}",
            request=request,
        )

        return redirect(reverse("users:ban_operation"))

    approve_appeal.short_description = "Разбанить"  # type: ignore[attr-defined]
    ...
    reject_appeal.short_description = "Отклонить"  # type: ignore[attr-defined]
    ...
    def get_urls(self) -> list[Any]:
        ...
        custom_urls = [
            path(
                "<int:appeal_id>/unban/",
                self.admin_site.admin_view(self.unban_view),
                name="users_appeal_unban",
            ),
            path(
                "<int:appeal_id>/reject/",
                self.admin_site.admin_view(self.reject_view),
                name="users_appeal_reject",
            ),
        ]
        return custom_urls + urls

    def unban_view(self, request: HttpRequest, appeal_id: int) -> HttpResponse:
